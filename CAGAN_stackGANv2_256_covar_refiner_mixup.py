...
def showG(A, cycle0_generate, cycle1_generate, cycle_generate):
    def G(fn_generate, X):
...
def main():
    ...
    _ = tf.InteractiveSession(config=tf.ConfigProto(gpu_options=gpu_options))
    ...
    netD0 = BASIC_D(nc_D_inp, ndf_train, use_sigmoid = not use_lsgan)
    netD1 = BASIC_D(nc_D_inp, ndf_train, use_sigmoid = not use_lsgan)
    netD2 = BASIC_D(nc_D_inp, ndf_train, use_sigmoid = not use_lsgan)
    netD2.summary()

    # ============Cycle GAN  - CAGAN - Stack GAN++ - Definition============
    ...
    res_folder = '/media/dev_hdd2/ramya/vuemodel/model_gen_trials/Conditional-Analogy-GAN-keras_results/iter2'
    display_iters = 50
    ...
    while gen_iterations < 8008:
        ...
        if gen_iterations%display_iters==0:
            ...
            showG(A, cycle0_generate, cycle1_generate, cycle_generate)
            filepath = "saved-model-epoch{}-iter{}-gen_loss{}.hdf5".format(str(epoch), str(gen_iterations), str(round(errGA_sum/display_iters, 3)))
            print ('Filepath : ', filepath)
            netGA.save(os.path.join(res_folder, filepath))
            errCyc_sum = errGA_sum = errDA_sum = errC_sum = 0
